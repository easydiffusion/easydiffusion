...
const imageModal = (function() {
  const zoomElem = createElement("i", undefined, [
    "fa-solid",
    "tertiaryButton"
  ]);

  const closeElem = createElement("i", undefined, [
    "fa-solid",
    "fa-xmark",
    "tertiaryButton"
  ]);

  const menuBarElem = createElement("div", undefined, "menu-bar", [
    zoomElem,
    closeElem
  ]);
  ...
  const modalContainer = createElement("div", undefined, "content", [
    menuBarElem,
    imageContainer
  ]);
  ...
  const setZoomLevel = value => {
    ...
    if (value) {
      ...
      if (img) {
        ...
        let zoomLevel =
          typeof value === "number" ? value : img.dataset.zoomLevel;
        if (!zoomLevel) {
          zoomLevel = 100;
        }
  ...
  zoomElem.addEventListener("click", () =>
    setZoomLevel(
      imageContainer.querySelector("img")?.classList?.contains("natural-zoom")
    )
  );
