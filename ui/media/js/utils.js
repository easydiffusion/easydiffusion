...
function toggleCollapsible(element) {
  ...
  document.dispatchEvent(
    new CustomEvent("collapsibleClick", { detail: collapsibleHeader })
  );
...
function saveCollapsibles() {
  ...
  COLLAPSIBLE_PANELS.forEach(element => {
    let value =
      element.querySelector(".collapsible").className.indexOf("active") !== -1;
...
function createCollapsibles(node) {
  ...
  if (save) {
    ...
    COLLAPSIBLE_PANELS.forEach(element => {
      let value =
        element.querySelector(".collapsible").className.indexOf("active") !==
        -1;
...
function BraceExpander() {
  ...
  function andTree(dctSofar, tkns) {
    ...
    let dctParse = dctSofar
      ...
      lstOR =
        dctBrace && dctBrace.close && dctBrace.commas.length
  ...
  function tokens(str) {
    ...
    return str
      ...
      .reduce(function(a, s) {
        return a.concat(
          s.charAt(0) === "\\" ? s : s.split(/(\\*[{,}])/).filter(toS)
        );
      }, []);
  }
...
function makeQuerablePromise(promise) {
  ...
  if (
    "isResolved" in promise ||
    "isRejected" in promise ||
    "isPending" in promise
  ) {
    return promise;
  }
  let isPending = true;
...
class GenericEventSource {
  ...
  constructor(...eventsTypes) {
    if (
      Array.isArray(eventsTypes) &&
      eventsTypes.length === 1 &&
      Array.isArray(eventsTypes[0])
    ) {
...
function createElement(tagName, attributes, classes, textOrElements) {
  ...
  if (textOrElements) {
    const children = Array.isArray(textOrElements)
      ? textOrElements
      : [textOrElements];
