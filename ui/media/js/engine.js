...
(function() {
    ...
    *readStreamAsJSON(jsonStr, throwOnError) {
      ...
      do {
        ...
        try {
          ...
          if (throwOnError) {
            console.error(
              `Parsing: "${this.#bufferedString}", Buffer: "${jsonStr}"`
            );
          }
          this.#bufferedString += jsonStr;
        ...
        this.#bufferedString.length > 0 &&
        this.#bufferedString.indexOf("}") >= 0
      );
    }
  }
  ...
  function setServerStatus(msgType, msg) {
    return eventSource.fireEvent(EVENT_STATUS_CHANGED, {
      type: msgType,
      message: msg
    });
  }
  ...
  async function healthCheck() {
    if (
      Date.now() < serverState.time + HEALTH_PING_INTERVAL / 2 &&
      isServerAvailable()
    ) {
    ...
    if (Date.now() >= serverState.time + SERVER_STATE_VALIDITY_DURATION) {
      console.warn(
        "WARNING! SERVER_STATE_VALIDITY_DURATION has elapsed since the last Ping completed."
      );
    }
    try {
      let res = undefined;
      ...
      switch (serverState.status) {
        ...
        default:
          // Unavailable
          console.error(
            "Ping received an unexpected server status. Status: %s",
            serverState.status
          );
  ...
  function isServerAvailable() {
    if (typeof serverState !== "object") {
      console.error(
        "serverState not set to a value. Connection to server could be lost..."
      );
      return false;
    }
    if (Date.now() >= serverState.time + SERVER_STATE_VALIDITY_DURATION) {
      console.warn(
        "SERVER_STATE_VALIDITY_DURATION elapsed. Connection to server could be lost..."
      );
      return false;
    }
    switch (serverState.status) {
      ...
      default:
        console.warn(
          "Unexpected server status. Server could be unavailable... Status: %s",
          serverState.status
        );
        return false;
    }
  ...
  async function waitUntil(isReadyFn, delay, timeout) {
    ...
    if (Date.now() >= serverState.time + SERVER_STATE_VALIDITY_DURATION) {
      console.warn(
        "SERVER_STATE_VALIDITY_DURATION elapsed. Released waitUntil on stale server state."
      );
    }
  }
  ...
  class Task {
    ...
    _setStatus(status) {
      ...
      if (currentIdx < 0) {
        throw Error(
          `The task status ${this.#status} is final and can't be changed.`
        );
      }
    ...
    async post(url, timeout = -1) {
      ...
      try {
        ...
        do {
          ...
          res.status === 503 &&
          (await asyncDelay(RETRY_DELAY_IF_SERVER_IS_BUSY))
        );
      } catch (err) {
        this.abort(err);
        throw err;
      ...
      if (!res.ok) {
        const err = new Error(
          `Unexpected response HTTP${res.status}. Details: ${res.statusText}`
        );
        this.abort(err);
        throw err;
      }
    ...
    async waitUntil({ timeout = -1, callback, status, signal }) {
      ...
      switch (this.#status) {
        ...
        case TaskStatus.waiting:
          ...
          if (
            stIdx >= 0 &&
            stIdx <= TASK_STATUS_ORDER.indexOf(TaskStatus.waiting)
          ) {
            return true;
          }
          ...
          const state =
            typeof serverState.tasks === "object"
              ? serverState.tasks[String(task.#id)]
              : undefined;
          if (
            state === "running" ||
            state === "buffer" ||
          ...
          if (
            stIdx >= 0 &&
            stIdx <= TASK_STATUS_ORDER.indexOf(TaskStatus.processing)
          ) {
            return true;
          }
    ...
    async enqueue(promiseGenerator, ...args) {
      if (this.status !== TaskStatus.init) {
        throw new Error(
          `Task is in an invalid status ${this.status} to add to queue.`
        );
      }
      this._setStatus(TaskStatus.pending);
    ...
    static async enqueue(promiseGenerator, ...args) {
      if (typeof promiseGenerator === "undefined") {
        throw new Error(
          "To enqueue a concurrent task, a *Promise Generator is needed but undefined was found."
        );
      }
      ...
      if (typeof promiseGenerator === "function") {
        concurrent_generators.set(
          asGenerator({ callback: promiseGenerator }),
      ...
      await waitUntil(
        () => !concurrent_generators.has(promiseGenerator),
        CONCURRENT_TASK_INTERVAL
      );
    ...
    static async run(
      ...
      do {
        ...
        if (callback) {
          ({ value, done } = await Promise.resolve(
            callback.call(promiseGenerator, { value, done })
          ));
        }
        if (value instanceof Promise) {
          value = await value;
    ...
    static async *asGenerator({
      ...
      do {
        ...
        if (callback) {
          ({ value, done } = await Promise.resolve(
            callback.call(generator, { value, done })
          ));
          if (value instanceof Promise) {
            value = await value;
          }
  ...
  class RenderTask extends Task {
    constructor(options = {}) {
      ...
      if (this._reqBody.init_image) {
        if (typeof this._reqBody.prompt_strength === "undefined") {
          ...
          throw new Error(
            `prompt_strength need to be of type number but ${typeof this
              ._reqBody.prompt_strength} was found.`
          );
        }
      }
      if ("modifiers" in this._reqBody) {
        if (
          Array.isArray(this._reqBody.modifiers) &&
          this._reqBody.modifiers.length > 0
        ) {
          this._reqBody.modifiers = this._reqBody.modifiers.filter(val =>
            val.trim()
          );
          if (this._reqBody.modifiers.length > 0) {
            this._reqBody.prompt = `${
              this._reqBody.prompt
            }, ${this._reqBody.modifiers.join(", ")}`;
          }
        }
        if (
          typeof this._reqBody.modifiers === "string" &&
          this._reqBody.modifiers.length > 0
        ) {
    ...
    checkReqBody() {
      ...
      for (const key in TASK_REQUIRED) {
        if (typeof this._reqBody[key] !== TASK_REQUIRED[key]) {
          throw new Error(
            `${key} need to be of type ${TASK_REQUIRED[key]} but ${typeof this
              ._reqBody[key]} was found.`
          );
        }
      }
      for (const key in this._reqBody) {
        ...
        if (key in TASK_OPTIONAL) {
          if (typeof this._reqBody[key] == "undefined") {
            ...
            console.warn(
              `reqBody[${key}] was set to undefined. Removing optional key without value...`
            );
            continue;
          }
          if (typeof this._reqBody[key] !== TASK_OPTIONAL[key]) {
            throw new Error(
              `${key} need to be of type ${TASK_OPTIONAL[key]} but ${typeof this
                ._reqBody[key]} was found.`
            );
          }
        }
    ...
    async post(timeout = -1) {
      ...
      if (typeof jsonResponse?.task !== "number") {
        ...
        if (typeof jsonResponse?.task !== "number") {
          const err = new Error(
            jsonResponse?.detail ||
              "Endpoint response does not contains a task ID."
          );
          this.abort(err);
          throw err;
        }
    ...
    *start(progressCallback) {
      if (
        ...
        throw new Error(
          "progressCallback is not a function. progressCallback type: " +
      ...
      const taskState =
        typeof serverState.tasks === "object"
          ? serverState.tasks[String(this.id)]
          : undefined;
      switch (taskState) {
        case "pending": // Session has pending tasks.
          console.error(
            "Server %o render request %o is still waiting.",
            serverState,
            renderRequest
          );
        ...
        case "completed":
          ...
          console.warn(
            "Server %o render request %o completed unexpectedly",
            serverState,
            renderRequest
          );
        ...
        case "error":
          ...
          console.error(
            "Server %o render request %o has failed",
            serverState,
            renderRequest
          );
        ...
        case "stopped":
          ...
          console.log(
            "Server %o render request %o was stopped",
            serverState,
            renderRequest
          );
          return false;
        default:
          if (!progressCallback) {
            const err = new Error(
              "Unexpected server task state: " + taskState || "Undefined"
            );
            this.abort(err);
            throw err;
          }
  ...
  function getServerCapacity() {
    let activeDevicesCount = Object.keys(serverState?.devices?.active || {})
      .length;
    if (
  ...
  function continueTasks() {
    if (typeof navigator?.scheduling?.isInputPending === "function") {
      const inputPendingOptions = {
        ...
        includeContinuous: Boolean(
          task_queue.size <= 0 && concurrent_generators.size <= 0
        )
      };
    ...
    if (task_queue.size < serverCapacity) {
      if (!idleEventPromise?.isPending) {
        idleEventPromise = makeQuerablePromise(
          eventSource.fireEvent(EVENT_IDLE, {
            capacity: serverCapacity - task_queue.size
          })
        );
      }
    }
    ...
    for (let [generator, promise] of concurrent_generators.entries()) {
      ...
      if (value instanceof Promise) {
        promise = makeQuerablePromise(
          value.then(val => ({ done: promise.resolvedValue?.done, value: val }))
        );
    ...
    for (let [task, generator] of task_queue.entries()) {
      ...
      const promise = makeQuerablePromise(
        beforeStart.then(() => Promise.resolve(event.beforeStart))
      );
  ...
  Object.defineProperties(getGlobal(), {
    ...
    sessionId: {
      ...
      get: () => {
        console.warn(
          "Deprecated window.sessionId has been replaced with SD.sessionId."
        );
        console.trace();
        return SD.sessionId;
      },
