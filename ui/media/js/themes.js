      ...
      if (
        ...
        if (DEFAULT_THEME) {
          ...
          Array.from(DEFAULT_THEME.rule.style)
            .filter(
              cssVariable => !Array.from(rule.style).includes(cssVariable)
            )
            .forEach(cssVariable => {
              rule.style.setProperty(
                cssVariable,
                DEFAULT_THEME.rule.style.getPropertyValue(cssVariable)
              );
            });
        }
...
function themeFieldChanged() {
  ...
  if (theme) {
    borderColor = theme.rule.style
      .getPropertyValue("--input-border-color")
      .trim();
    if (!borderColor.startsWith("#")) {
      borderColor = theme.rule.style.getPropertyValue("--theme-color-fallback");
    }
  } else {
    borderColor = DEFAULT_THEME.rule.style.getPropertyValue(
      "--theme-color-fallback"
    );
  }
  document
    .querySelector('meta[name="theme-color"]')
    .setAttribute("content", borderColor);
}
