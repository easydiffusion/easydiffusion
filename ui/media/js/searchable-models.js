...
class ModelDropdown {
  ...
  processClick(e) {
    e.preventDefault();
    if (
      e.srcElement.classList.contains("model-file") ||
      e.srcElement.classList.contains("fa-file")
    ) {
      const elem = e.srcElement.classList.contains("model-file")
        ? e.srcElement
  ...
  getFirstVisibleChild(elem) {
    ...
    if (firstElementChild.style.display == "list-item")
  ...
  selectNextFile() {
    this.selectModelEntry(
      this.getNextVisibleSibling(this.highlightedModelEntry)
    );
    this.modelFilter.select();
  }
  ...
  filterList(e) {
    ...
    this.modelList.querySelectorAll("li").forEach(function(li) {
      ...
      if (filter == "") {
        ...
        showAllChildren ||
        li.textContent.toLowerCase().match(filter)
      ) {
        li.style.display = "list-item";
        if (
          li.classList.contains("model-folder") &&
          li.firstChild.textContent.toLowerCase().match(filter)
        ) {
    ...
    if (found) {
      ...
      this.modelNoResult.style.display = "none";
      const elem = this.getNextVisibleSibling(
        this.modelList.querySelector(".model-file")
      );
      this.highlightModel(elem);
      elem.scrollIntoView({ block: "nearest" });
    } else {
  ...
  createDropdown() {
    ...
    this.modelFilterArrow = document.querySelector(
      `#${this.modelFilter.id}-model-filter-arrow`
    );
    if (this.modelFilterArrow) {
      this.modelFilterArrow.style.color = this.modelFilter.disabled
        ? "dimgray"
        : "";
    }
    this.modelList = document.querySelector(
      `#${this.modelFilter.id}-model-list`
    );
    this.modelResult = document.querySelector(
      `#${this.modelFilter.id}-model-result`
    );
    this.modelNoResult = document.querySelector(
      `#${this.modelFilter.id}-model-no-result`
    );

    if (this.modelFilterInitialized !== true) {
      this.modelFilter.addEventListener(
        "input",
        this.bind(this.filterList, this)
      );
      this.modelFilter.addEventListener(
        "focus",
        this.bind(this.modelListFocus, this)
      );
      this.modelFilter.addEventListener(
        "blur",
        this.bind(this.hideModelList, this)
      );
      this.modelFilter.addEventListener(
        "click",
        this.bind(this.showModelList, this)
      );
      this.modelFilter.addEventListener(
        "keydown",
        this.bind(this.processKey, this)
      );

      this.modelFilterInitialized = true;
    }
    this.modelFilterArrow.addEventListener(
      "mousedown",
      this.bind(this.toggleModelList, this)
    );
    this.modelList.addEventListener(
      "mousemove",
      this.bind(this.highlightModelAtPosition, this)
    );
    this.modelList.addEventListener(
      "mousedown",
  ...
  createRootModelList(modelTree) {
    ...
    if (modelTree.length > 0) {
      ...
      containerListItem.appendChild(
        this.createModelNodeList(undefined, modelTree, true)
      );
