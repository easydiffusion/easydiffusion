...
const IMAGE_EDITOR_TOOLS = [
  ...
  },
  {
    ...
    begin: (editor, ctx, x, y, is_overlay = false) => {
      if (!is_overlay) {
        var img_rgb = editor.layers.background.ctx.getImageData(x, y, 1, 1)
          .data;
    ...
    handler: editor => {
      ...
      el.addEventListener("change", function() {
        ...
        reader.addEventListener("load", function(event) {
          ...
          editor.layers.drawing.ctx.clearRect(
            0,
            0,
            editor.width,
            editor.height
          ...
          var image = new Image();
          image.onload = () => {
            editor.layers.drawing.ctx.drawImage(
              image,
              0,
              0,
              editor.width,
...
class EditorHistory {
  ...
  push(event) {
    ...
    if (
      ...
      this.events.length % snapshot_frequency == 0
    ) {
      event.snapshot = this.editor.layers.drawing.ctx.getImageData(
        0,
        0,
        this.editor.width,
        this.editor.height
      ...
      if (event.type == "action") {
        ...
        for (var point_i = 1; point_i < event.points.length; point_i++) {
          tool.move(
            this.editor,
            ctx,
            event.points[point_i].x,
            event.points[point_i].y
          );
        }
...
class ImageEditor {
  constructor(popup, inpainter = false) {
    ...
    IMAGE_EDITOR_SECTIONS.forEach(section => {
      ...
      section.options.forEach((option, index) => {
        ...
        optionElement.addEventListener("click", target =>
          this.selectOption(section.name, index)
        );
      ...
      this.popup
        .querySelector(".editor-controls-left")
    ...
    if (this.inpainter) {
      this.selectOption(
        "color",
        IMAGE_EDITOR_SECTIONS.find(s => s.name == "color").options.indexOf(
          "#ffffff"
        )
      );
      this.selectOption(
        "opacity",
        IMAGE_EDITOR_SECTIONS.find(s => s.name == "opacity").options.indexOf(
          0.4
        )
      );
    }
    ...
    this.popup
      .querySelector(".editor-controls-right")
  ...
  setImage(url, width, height) {
    ...
    if (url) {
      var image = new Image();
      image.onload = () => {
        this.layers.background.ctx.drawImage(
          image,
          0,
          0,
          this.width,
  ...
  getOptionValue(section_name) {
    ...
    return this.options && section_name in this.options
  ...
  selectOption(section_name, option_index) {
    ...
    this.options[section_name] =
      value == "custom" ? section.getCustom(this) : value;

    this.optionElements[section_name].forEach(element =>
...
const imageInpainter = new ImageEditor(
  document.getElementById("image-inpainter"),
  true
);
...
document
  .getElementById("init_image_button_draw")
  .addEventListener("click", () => {
    imageEditor.show();
  });
document
  .getElementById("init_image_button_inpaint")
  .addEventListener("click", () => {
    imageInpainter.show();
  });
