  ...
  res.output.forEach((result, index) => {
        ...
        shiftOrConfirm(e, "Remove the image from the results?", () => {
          ...
          if (allHidden === true) {
            ...
            if (!req.isProcessing || req.batchesDone == req.batchCount) {
              parentTaskContainer.parentNode.removeChild(parentTaskContainer);
            }
          }
        });
      });
    ...
    if ("seed" in result && !imageElem.hasAttribute("data-seed")) {
        ...
        [
          {
            ...
            filter: (req, img) => !req.use_upscale
          },
          {
            text: "Fix Faces",
            on_click: onFixFacesClick,
            filter: (req, img) => !req.use_face_correction
          }
        ]
      ];
      ...
      const createButton = function(btnInfo) {
        ...
        if (btnInfo.html) {
          const html =
            typeof btnInfo.html === "function" ? btnInfo.html() : btnInfo.html;
          ...
          newButton.innerText =
            typeof btnInfo.text === "function" ? btnInfo.text() : btnInfo.text;
        }
      ...
      buttons.forEach(btn => {
        if (Array.isArray(btn)) {
          btn = btn.filter(
            btnInfo => !btnInfo.filter || btnInfo.filter(req, img) === true
          );
          if (btn.length === 0) {
            return;
          }
...
function getUncompletedTaskEntries() {
  const taskEntries = Array.from(
    ...
    .map(function(taskLabel) {
      ...
      while (
        !imageTaskContainer.classList.contains("imageTaskContainer") &&
        imageTaskContainer.parentNode
      ) {
  ...
  return async function(event) {
      ...
      switch (this.status) {
        ...
        case SD.TaskStatus.failed:
          if (!SD.isServerAvailable()) {
            ...
            let msg =
              "Stable Diffusion had an error reading the response:<br/><pre>";
            if (this.exception) {
    ...
    if ("update" in event) {
      ...
      const timeRemaining =
        timeTaken < 0
          ? ""
          : millisecondsToStr(stepsRemaining * timeTaken * 1000);
...
function onTaskErrorHandler(task, reqBody, instance, reason) {
  ...
  console.log(
    "Render request %o, Instance: %o, Error: %s",
    reqBody,
    instance,
    reason
  );
...
function onTaskCompleted(task, reqBody, instance, outputContainer, stepUpdate) {
  ...
  if (
    "instances" in task &&
    task.instances.some(ins => ins != instance && ins.isPending)
  ) {
    return;
  }
...
async function onTaskStart(task) {
  ...
  if (task.batchCount > 1) {
    ...
    if (task.batchesDone == task.batchCount - 1) {
      ...
      newTaskReqBody.num_outputs =
        task.numOutputsTotal - task.reqBody.num_outputs * (task.batchCount - 1);
    }
  }

  const startSeed = task.seed || newTaskReqBody.seed;
  const genSeeds = Boolean(
    typeof newTaskReqBody.seed !== "number" ||
      (newTaskReqBody.seed === task.seed && task.numOutputsTotal > 1)
  );
  if (genSeeds) {
    newTaskReqBody.seed =
      parseInt(startSeed) + task.batchesDone * task.reqBody.num_outputs;
  }
  ...
  task.outputContainer.insertBefore(
    outputContainer,
    task.outputContainer.firstChild
  );
  ...
  const callbacksPromises = PLUGINS["TASK_CREATE"].map(hook => {
    if (typeof hook !== "function") {
      console.error(
        "The provided TASK_CREATE hook is not a function. Hook: %o",
        hook
      );
  ...
  if (!instance) {
    const factory = PLUGINS.OUTPUTS_FORMATS.get(
      eventInfo.reqBody?.output_format || newTaskReqBody.output_format
    );
    if (factory) {
      instance = await Promise.resolve(
        factory(eventInfo.reqBody || newTaskReqBody)
      );
    }
    if (!instance) {
      console.error(
        `${
          factory ? "Factory " + String(factory) : "No factory defined"
        } for output format ${eventInfo.reqBody?.output_format ||
          newTaskReqBody.output_format}. Instance is ${instance ||
...
function createTask(task) {
  ...
  if (task.reqBody.use_upscale) {
    taskConfig += `, <b>Upscale:</b> ${task.reqBody.use_upscale} (${task.reqBody
      .upscale_amount || 4}x)`;
  }
  ...
  task["stopTask"].addEventListener("click", e => {
    ...
    let question = task["isProcessing"]
      ? "Stop this task?"
      : "Remove this task?";
