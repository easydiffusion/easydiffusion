...
const TASK_MAPPING = {
  ...
  inactive_tags: {
    ...
    readUI: () =>
      activeTags.filter(tag => tag.inactive === true).map(x => x.name),
    parse: val => val
  },
  width: {
    name: "Width",
  ...
  preserve_init_image_color_profile: {
    ...
    setUI: preserve_init_image_color_profile => {
      applyColorCorrectionField.checked = parseBoolean(
        preserve_init_image_color_profile
      );
    },
  ...
  use_face_correction: {
    ...
    readUI: () =>
      useFaceCorrectionField.checked ? gfpganModelField.value : undefined,
    parse: val => val
  },
  use_upscale: {
    name: "Use Upscaling",
    ...
    readUI: () =>
      useUpscalingField.checked ? upscaleModelField.value : undefined,
    parse: val => val
  },
  upscale_amount: {
    name: "Upscale By",
  ...
  use_stable_diffusion_model: {
    ...
    setUI: use_stable_diffusion_model => {
      ...
      use_stable_diffusion_model = getModelPath(use_stable_diffusion_model, [
        ".ckpt",
        ".safetensors"
      ]);
  ...
  stream_image_progress: {
    ...
    setUI: stream_image_progress => {
      streamImageProgressField.checked =
        parseInt(numOutputsTotalField.value) > 50
          ? false
          : stream_image_progress;
    },
...
function restoreTaskToUI(task, fieldsToSkip) {
  ...
  if (
    IMAGE_REGEX.test(initImagePreview.src) &&
...
function parseTaskFromText(str) {
  ...
  for (const key in TASK_TEXT_MAPPING) {
    ...
    if (match) {
      str =
        str.slice(0, match.index) + str.slice(match.index + match[0].length);
      val = match[1];
    }
    if (val !== undefined) {
...
function dropHandler(ev) {
  ...
  let text_items = items.filter(item =>
    TEXT_EXTENSIONS.includes(item.file_ext)
  );
  let image_items = items.filter(item =>
    IMAGE_EXTENSIONS.includes(item.file_ext)
  );
...
navigator.permissions
  ...
  .then(checkReadTextClipboardPermission, reason =>
    console.log("clipboard-read is not available. %o", reason)
  );
...
navigator.permissions
  ...
  .then(checkWriteToClipboardPermission, e => {
    if (
