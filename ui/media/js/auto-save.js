// Saving settings
let saveSettingsConfigTable = document.getElementById(
...
async function initSettings() {
  ...
  SETTINGS_SECTIONS.forEach(section => {
    ...
    var children =
      unsorted_ids == ""
        ? []
        : Array.from(element.querySelectorAll(unsorted_ids));
    section.keys = [];
    children.forEach(e => {
      section.keys.push(e.id);
    });
    unsorted_settings_ids = unsorted_settings_ids.filter(
      id => children.find(e => e.id == id) == undefined
    );
  });
  loadSettings();
}
...
function loadSettings() {
  ...
  if (saved_settings_text) {
    ...
    if (
      saved_settings.find(s => s.key == "auto_save_settings")?.value == false
    ) {
    ...
    saved_settings.forEach(saved_setting => {
      ...
      if (!setting) {
        console.warn(
          `Attempted to load setting ${saved_setting.key}, but no setting found`
        );
        return null;
      }
...
autoSaveSettings.addEventListener("change", () => {
  configSettingsButton.style.display = autoSaveSettings.checked
    ? "block"
    : "none";
});
...
function tryLoadOldSettings() {
  ...
  Object.keys(individual_settings_map).forEach(localStorageKey => {
    ...
    if (localStorageValue !== null) {
      ...
      if (!setting) {
        console.warn(
          `Attempted to map old setting ${key}, but no setting found`
        );
        return null;
      }
      if (
        ...
        (typeof localStorageValue === "string" ||
          localStorageValue instanceof String)
      ) {
