...
function createModifierCard(name, previews, removeBy) {
  ...
  if (cardLabel.length <= maxLabelLength) {
    ...
    label.querySelector("p").innerText =
      cardLabel.substring(0, maxLabelLength) + "...";
  }
...
function createModifierGroup(modifierGroup, initiallyExpanded, removeBy) {
  ...
  modifiers.forEach(modObj => {
    ...
    const modifierPreviews = modObj?.previews?.map(
      preview =>
        `${
          IMAGE_REGEX.test(preview.image)
            ? preview.image
            : modifierThumbnailPath + "/" + preview.path
        }`
    );

    const modifierCard = createModifierCard(
      modifierName,
      modifierPreviews,
      removeBy
    );
      ...
      modifierCard.addEventListener("click", () => {
        if (activeTags.map(x => trimModifiers(x.name)).includes(trimmedName)) {
          ...
          activeTags = activeTags.filter(
            x => trimModifiers(x.name) != trimmedName
          );
  ...
  editorModifierEntries.insertBefore(
    e,
    customModifierEntriesToolbar.nextSibling
  );
    ...
    if (res.status === 200) {
      ...
      res.forEach((modifierGroup, idx) => {
        createModifierGroup(
          modifierGroup,
          idx === res.length - 1,
          modifierGroup === "Artist" ? true : false
        ); // only remove "By " for artists
      });
...
function refreshModifiersState(newTags) {
  ...
  document
    ...
    .forEach(modifierCard => {
      const modifierName = modifierCard.querySelector(".modifier-card-label p")
        .dataset.fullName; // pick the full modifier name
  ...
  newTags.forEach(tag => {
    ...
    document
      ...
      .querySelectorAll(".modifier-card")
      .forEach(modifierCard => {
        const modifierName = modifierCard.querySelector(
          ".modifier-card-label p"
        ).dataset.fullName;
        const shortModifierName = modifierCard.querySelector(
          ".modifier-card-label p"
        ).innerText;
        if (trimModifiers(tag) == trimModifiers(modifierName)) {
          ...
          if (!activeTags.map(x => x.name).includes(tag)) {
            ...
            imageModifierCard.querySelector(
              ".modifier-card-label p"
            ).innerText = tag.replace(modifierName, shortModifierName);
            activeTags.push({
              name: tag,
...
function refreshInactiveTags(inactiveTags) {
  ...
  let overlays = document
    ...
    .querySelectorAll(".modifier-card-overlay");
  overlays.forEach(i => {
...
function changePreviewImages(val) {
  ...
  modifiers
    .map(x => x.modifiers)
    .forEach(x => previewArr.push(...x.map(m => m.previews)));
  ...
  previewImages.forEach(previewImage => {
    ...
    const relativePreviewPath = previewImage.src
      .split(modifierThumbnailPath + "/")
      .pop();

    const previews = previewArr.find(
      preview => relativePreviewPath == preview[currentPreviewType]
    );
...
function resizeModifierCards(val) {
  ...
  modifierCards.forEach(card => {
    ...
    const classes = card.className
      .split(" ")
      .filter(c => !c.startsWith(cardSizePrefix));
...
modifierCardSizeSlider.onchange = () =>
  resizeModifierCards(modifierCardSizeSlider.value);
...
function saveCustomModifiers() {
  localStorage.setItem(
    CUSTOM_MODIFIERS_KEY,
    customModifiersTextBox.value.trim()
  );
