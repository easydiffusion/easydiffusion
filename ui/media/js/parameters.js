...
var PARAMETERS = [
  ...
  },
  {
    ...
    label: "Network port",
    note:
      "Port that this server listens to. The '9000' part in 'http://localhost:9000'",
    icon: "fa-anchor",
    render: parameter => {
  ...
  switch (parameter.type) {
    ...
    case ParameterType.select_multiple:
      var options = (parameter.options || [])
        ...
        .join("");
      var multiple =
        parameter.type == ParameterType.select_multiple ? "multiple" : "";
...
let parametersTable = document.querySelector(
  "#system-settings .parameters-table"
);
...
let metadataOutputFormatField = document.querySelector(
  "#metadata_output_format"
);
let listenToNetworkField = document.querySelector("#listen_to_network");
let listenPortField = document.querySelector("#listen_port");
let useBetaChannelField = document.querySelector("#use_beta_channel");
let uiOpenBrowserOnStartField = document.querySelector(
  "#ui_open_browser_on_start"
);
let confirmDangerousActionsField = document.querySelector(
...
function setDeviceInfo(devices) {
  ...
  function ID_TO_TEXT(d) {
    ...
    if ("mem_free" in info && "mem_total" in info) {
      return `${info.name} <small>(${d}) (${info.mem_free.toFixed(
        1
      )}Gb free / ${info.mem_total.toFixed(1)} Gb total)</small>`;
    } else {
  ...
  systemInfoEl.querySelector(
    "#system-info-rendering-devices"
  ).innerHTML = activeGPUs.join("</br>");
}
...
async function getSystemInfo() {
  try {
    ...
    let activeDeviceIds = Object.keys(devices["active"]).filter(
      d => d !== "cpu"
    );
    ...
    if (
      allDeviceIds.length < MIN_GPUS_TO_SHOW_SELECTION ||
      useCPUField.checked
    ) {
      let gpuSettingEntry = getParameterSettingsEntry("use_gpus");
      gpuSettingEntry.style.display = "none";
