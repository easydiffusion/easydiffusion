...
export function HistoryItem(props: HistoryItemProps) {
	const {item} = props;
	const {response} = item;
	const showErrorState = !response || response.status !== 'succeeded' || !response.output.length;
	if (showErrorState) {
		console.warn('Unable to render a history item, maybe it failed to generate? Item: ', item);
	}
...
You will not be able to recover this data.`;
	...
	return (
		<div className="history-item" style={{maxWidth: `${item.width}px`, maxHeight: `${item.height}px`}}>
			{showErrorState && (

				<div>Unable to show this item. Maybe it failed to generate?<br/><small>Check browser devtools console
					for details of this item.</small></div>
			)}
			{output && (
				<>
					<button className="history-item__toggle-details-btn" type="button"
						...
						{itemDetailsAreVisible && (
							<div className="history-item__details">
								...
								<dl className="history-item__details-body">
									...
									<dd>{item.seed}</dd>
									<dt>dimensions</dt>
									<dd>{item.width}px * {item.height}px</dd>
								...
								<div className="history-item__details-actions">
									<button type="button" onClick={() => onDownloadImage()} title="Download image">
										<DownloadIcon label="Download image"/>
									</button>

									<button type="button" className="history-item__details-actions-delete"
											onClick={deleteItem} title="Delete this item">
