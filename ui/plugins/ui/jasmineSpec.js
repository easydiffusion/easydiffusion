  ...
  it("should be able to stream data in chunks", async function() {
    ...
    await SD.waitUntil(
      () => {
        ...
        return (
          !SD.serverState.tasks ||
          SD.serverState.tasks[String(renderRequest.task)]
        );
      },
      250,
      10 * 60 * 1000
    ...
    await SD.waitUntil(() => {
      ...
      return (
        !SD.serverState.tasks ||
        SD.serverState.tasks[String(renderRequest.task)] !== "pending"
      );
    }, 250);
      ...
      const result = await SD.render(
        ...
        function(event) {
          ...
          if ("update" in event) {
            ...
            if (
              complete ||
              (stepUpdate.status && stepUpdate.step === stepUpdate.total_steps)
            ) {
              expect(stepUpdate.status).toBe("succeeded");
...
loadUIPlugins().then(function() {
  ...
  describe("@Plugins", function() {
    ...
    describe("supports selftests", function() {
      ...
      const pluginsTests = Object.keys(PLUGINS.SELFTEST).filter(key =>
        PLUGINS.SELFTEST.hasOwnProperty(key)
      );
