(function() {
  ...
  let mergeModelAField = new ModelDropdown(
    document.querySelector("#mergeModelA"),
    "stable-diffusion"
  );
  let mergeModelBField = new ModelDropdown(
    document.querySelector("#mergeModelB"),
    "stable-diffusion"
  );
  updateChart();
  ...
  const singleMergeRatioSlider = document.querySelector(
    "#single-merge-ratio-slider"
  );
  ...
  document
    .querySelector(".merge-config")
    .addEventListener("change", updateChart);

  document
    .querySelector("#merge-button")
    .addEventListener("click", async function(e) {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        addLogMessage(
          `merging batch job ${i + 1}/${iterations}, alpha = ${alpha.toFixed(
            5
          )}...`
        );
        ...
        request["out_path"] +=
          ...
          alpha.toFixed(5) +
          "." +
          document.querySelector("#merge-format").value;
